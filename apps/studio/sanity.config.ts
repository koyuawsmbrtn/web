    ...
    structure: (S) =>
      S.list()
        ...
        .items([
          S.listItem()
            .title('Posts')
            .child(S.documentList().title('Blog posts').filter('_type == "post"').defaultOrdering([{field: 'publishedAt', direction: 'desc'}]))
            .icon(ComposeIcon),
          S.listItem()
            .title('Pages')
            .child(S.documentList().title('Pages').filter('_type == "page"').defaultOrdering([{field: 'sortOrder', direction: 'asc'}]))
            .icon(DocumentIcon),
          S.listItem()
            .title('Notes')
